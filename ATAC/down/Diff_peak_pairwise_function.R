...
pairwise_DESeq2 <- function(x) {
  ...
  pdf(plot_name)
  dba.plotMA(dba_diff,fold = Fold_cutoff, cex.main = 1, cex.lab = 1, cex.axis = 1)
  abline(h = c(-Fold_cutoff,Fold_cutoff),col = "#ec008c", lty = 5)
  dev.off()
  
  # report
  # dba_report_all <- dba.report(dba_diff,th = 1,method = DBA_DESEQ2_BLOCK)
  dba_report_all <- dba.report(dba_diff,th = 1)
  names(dba_report_all) <- paste0("GJ_peak_",names(dba_report_all))
