...
marker_file <- "./Result/SIM12d/Marker/MarkerPeaks_iter4.xlsx"
...
goall <- lapply(sheets, function(x) {
    ...
    gene <- unique(
        os_at[RAP_ID %in% markerList[[x]]$gene_id, TAIR]
    )
    if (length(gene) == 0) {
...
names(goall) <- sheets
# 保存
openxlsx::write.xlsx(goall, paste0(path_dir(marker_file), "/MarkerPeaks_go.xlsx"))
...
pall <- do.call(cowplot::plot_grid, c(list(ncol = 4), p))


pdf("Result/SIM12d/Plots/markerPeaks_go.pdf", height = 48, width = 36)
pall
dev.off()
