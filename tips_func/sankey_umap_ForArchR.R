Clu.sankey_ummap <- function(ArchR.proj = proj,
  ...
  if (sum(c(iter.Name1, iter.Name2) %in% names(cellColData)) != 2) {
    stop("? Please check the iter.Names: ", iter.Name1, "; ", iter.Name2)
  }
  if (sum(c(umap.Name1, umap.Name2) %in% names(embData)) != 2) {
    stop("? Please check the iter.Names: ", umap.Name1, "; ", umap.Name2)
  }

  col <- c(
    "#cc1616", "#d0595a", "#f786a8", "#631913",
  ...
  # umap1_color <- col[sample(seq.int(unique(umap1$Cluster)))]
  ...
  # umap2_color <- col[sample(seq.int(unique(umap2$Cluster)))]
